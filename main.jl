...
for i in eachindex(FileList)
   ...
   if BPP.NBvariables < 1000 && BPP.NBconstraints < 1000
      tic()
      println("Pour le probleme : ",FileList[i])
      cs = CurrentSolution(BPP.NBconstraints, BPP.NBvariables, 0, 0, BPP.Variables,zeros(BPP.NBvariables), BPP.LeftMembers_Constraints, zeros(BPP.NBconstraints), zeros(3,BPP.NBvariables), BPP.LeftMembers_Constraints)
      cs = FindingAdmissingBaseSolution1(cs)
